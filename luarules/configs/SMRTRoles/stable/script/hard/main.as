...
namespace Main {
	...
	{	
		...
		ApplyUnitLimits(selectedMapConfig.UnitLimits);
	...
	void ApplyUnitLimits(dictionary@ unitLimits) {
		...
		for (uint i = 0; i < keys.length(); ++i) {
			...
			if (unitLimits.get(unitName, maxLimit)) {  
				CCircuitDef@ circuitDef = ai.GetCircuitDef(unitName);  
				if (circuitDef !is null) { 
					LogUtil("Set Unit Limit:" + unitName + ":" + maxLimit, 2);
					circuitDef.maxThisUnit = maxLimit;  
				} else {
					LogUtil("Warning: Unit not found: " + unitName, 2); 
				}
			}
		}
	}
