...
class MapConfig {  
    ...
    bool CheckMatch(string mapName) {
		LogUtil("CheckMatch:MapName:" + mapName, 2);
		LogUtil("CheckMatch:_mapNameMatch:" + _mapNameMatch, 2);
    ...
    MapConfig getMapConfig(string mapName) {
        ...
        for (uint i = 0; i < mapConfigs.length(); i++) {
			if (mapConfigs[i] is null) {
				LogUtil("getMapConfig() mapConfigs[" + i + "] is null!", 3);
				continue;
			}
            ...
            if (mapConfigs[i].CheckMatch(mapName)) {
				LogUtil("getMapConfig() return mapConfigs[i]; " + mapName, 1);
                return mapConfig;
            }
        }
