...
namespace RoleSea {
    ...
    IUnitTask@ Sea_FactoryAiMakeTask(CCircuitUnit@ u)
    {
        ...
        if (isT1Shipyard) {
            ...
            if (Global::RoleSettings::Sea::EnableEarlyRezSub) {
                ...
                // Side IDs: armada -> armrecl, cortex -> correcl; Legion falls back to Cortex variant
                string rezSub = (side == "armada" ? "armrecl" : (side == "cortex" ? "correcl" : "correcl"));
    ...
    ******************************************************************************/ 
    ...
    if (udef is null) return Builder::MakeDefaultTaskWithLog(builder.id, "SEA");

        // Specialize only T1 sea constructors; everything else falls back to default
        string uname = udef.GetName();
        bool isT1SeaConstructor = (uname.length() >= 2 && uname.substr(uname.length() - 2, 2) == "cs");
        ...
        if (isT1SeaConstructor) {
            return Sea_T1Constructor_AiMakeTask(builder);
        } else if (isT2SeaConstructor) {
            return Sea_T2Constructor_AiMakeTask(builder);
        }

    // Fallback to default with logging
    return Builder::MakeDefaultTaskWithLog(builder.id, "SEA");
    }
    ...
    ******************************************************************************/
    ...
    IUnitTask@ Sea_T1Constructor_AiMakeTask(CCircuitUnit@ u) {
        // Snapshot economy
        ...
        if (u is Builder::primaryT1SeaConstructor) {
           ...
           // }
            ...
            if (EconomyHelpers::ShouldBuildT1EnergyConverter(
                mi,
                ei,
                Global::Economy::EnergyCurrent,
                Global::Economy::EnergyStorage,
    ...
    // Default fallback
    return Builder::MakeDefaultTaskWithLog(u.id, "SEA");
    }

    IUnitTask@ Sea_T2Constructor_AiMakeTask(CCircuitUnit@ u) {
        // Snapshot economy
        float mi = Global::Economy::MetalIncome;
        float ei = Global::Economy::EnergyIncome;
        bool isEnergyFull = aiEconomyMgr.isEnergyFull;
        ...
        if (u is Builder::primaryT2SeaConstructor) {
            bool isEnergyLessThan90Percent = Global::Economy::EnergyCurrent < Global::Economy::EnergyStorage * Global::RoleSettings::Sea::EnergyStorageLowPercent;
