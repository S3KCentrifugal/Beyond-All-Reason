...
namespace RoleFront {
    ...
    ******************************************************************************/ 
    ...
    if (udef is null) return Builder::MakeDefaultTaskWithLog(builder.id, "FRONT");
        ...
        if (ctorTier == 1) {
            if (builder is Builder::primaryT1BotConstructor || builder is Builder::secondaryT1BotConstructor
             || builder is Builder::primaryT1VehConstructor || builder is Builder::secondaryT1VehConstructor) {
                ...
                return Front_T1Constructor_AiMakeTask(builder, metalIncome, energyIncome, isEnergyStalling, isEnergyFull);
            }
        } else if (ctorTier == 2) {
            ...
            float metalIncome = Economy::GetMinMetalIncomeLast10s();
            float energyIncome = Economy::GetMinEnergyIncomeLast10s();
            float metalCurrent = Global::Economy::MetalCurrent;
            bool isEnergyLessThan90Percent = Global::Economy::EnergyCurrent < Global::Economy::EnergyStorage * Global::RoleSettings::Tech::EnergyStorageLowPercent;
            if (builder is Builder::primaryT2BotConstructor || builder is Builder::secondaryT2BotConstructor || builder is Builder::freelanceT2BotConstructor) {
                return Front_T2Constructor_AiMakeTask(builder, isEnergyFull, metalIncome, energyIncome, metalCurrent, isEnergyLessThan90Percent);
            }
        }
    // Fallback to default with logging
    return Builder::MakeDefaultTaskWithLog(builder.id, "FRONT");
    }
    ...
    ******************************************************************************/ 
    ...
    IUnitTask@ Front_T1Constructor_AiMakeTask(CCircuitUnit@ u, float metalIncome, float energyIncome, bool isEnergyStalling, bool isEnergyFull) {
        ...
        if (u is Builder::primaryT1BotConstructor) {
            ...
            if (t2LabCount < 1) {
                ...
                if (t2LabDef !is null) {
                    storedMetalTriggerMet = (Global::Economy::MetalCurrent >= t2LabDef.costM);
                }
            ...
            bool shouldT2Lab = EconomyHelpers::ShouldBuildT2BotLab(
                /*mi*/ metalIncome,
                /*ei*/ energyIncome,
                /*metalCurrent*/ Global::Economy::MetalCurrent,
            ...
            {
                float energyPercent = (Global::Economy::EnergyStorage > 0.0f)
                    ? (Global::Economy::EnergyCurrent / Global::Economy::EnergyStorage)
                    : 0.0f;
        ...
        if (u is Builder::primaryT1VehConstructor) {
            ...
            return Builder::MakeDefaultTaskWithLog(u.id, "FRONT");
        }

    return Builder::MakeDefaultTaskWithLog(u.id, "FRONT");
    }

    IUnitTask@ Front_T2Constructor_AiMakeTask(CCircuitUnit@ u, bool isEnergyFull, float metalIncome, float energyIncome, float metalCurrent, bool isEnergyLessThan90Percent) {
