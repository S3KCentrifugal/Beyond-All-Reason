...
namespace UnitHelpers {
    ...
    *************************************************************************************/
    ...
    // Note: This list purposefully excludes constructors, utility ships (e.g., MLS/radar), AA/jammer ships (typically T2), and transports.
    // TODO: Revisit against upstream unit sets periodically; extend if additional T1 naval units are confirmed.
    array<string> GetAllT1NavalCombatUnits()
    {
        array<string> ids; dictionary seen;

        // Armada (Arm) – T1 sea combat
        // armpt: Patrol boat (light AA/sonar)
        // armsub: Submarine
        // armroy: Destroyer
        {
            array<string> arm = { "armpt", "armsub", "armroy" };
        ...
        // Cortex (Core) – T1 sea combat
        // corpt: Patrol boat
        // corsub: Submarine
        // corroy: Destroyer
        {
            array<string> cor = { "corpt", "corsub", "corroy" };
        ...
        // Legion – T1 sea combat
        // legvelite: Light Corvette
        // legpontus: Anti-submarine scout craft
        // legtriarius: Destroyer (Legion)
        {
            array<string> leg = { "legvelite", "legpontus", "legtriarius" };
    ...
    {
        // Armada/Cortex have dedicated naval converters; Legion sea often maps to Cortex until finalized
        if (side == "armada") return "armfmkr";
        if (side == "cortex") return "corfmkr";
        if (side == "legion") return "corfmkr"; // NOTE: Legion currently reuses Cortex naval converter; swap to 'legfmkr' if validated upstream
    ...
    string GetTidalNameForSide(const string &in side)
    {
        ...
        // Legion reuses cortex sea units for now until game development concludes
        if (side == "legion") return "cortide";
